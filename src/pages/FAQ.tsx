...
export default function FAQ() {
  ...
  return (
    <>
      <SEOHead
        title={`${t('faq.title')} | Meca Store`}
        description={t('faq.subtitle')}
        canonical="https://mecastorepy.lovable.app/faq"
        keywords="FAQ meca store, envíos paraguay, política de cambios"
        jsonLd={[
      ...
      <main className="min-h-screen bg-background">
        <section className="bg-gradient-to-b from-primary/5 to-background pt-28 pb-16">
          <div className="container">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-2xl mx-auto">
              <h1 className="font-display text-4xl md:text-5xl font-bold text-foreground mb-4">{t('faq.title')}</h1>
        ...
        <section className="py-16">
          <div className="container max-w-3xl">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="mt-12 text-center bg-card border border-border rounded-xl p-8">
              <h2 className="font-display text-xl font-semibold text-foreground mb-2">{t('faq.needHelp')}</h2>
              <p className="text-muted-foreground mb-4">{t('faq.needHelpDesc')}</p>
              <div className="flex flex-wrap justify-center gap-3">
                <Link to="/catalogo" className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 transition">
                  <ShoppingBag className="h-4 w-4" />
                  {t('faq.viewCatalog')}
                </Link>
                <Link to="/atacado" className="inline-flex items-center gap-2 px-5 py-2.5 border border-border text-foreground rounded-lg font-medium hover:bg-muted transition">
                  {t('faq.wholesalePurchases')}
                </Link>
              </div>
            </motion.div>
          </div>
